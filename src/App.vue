...
import { getDailyLedger, getPriceProviderStatus, getRealm, saveSnapshot, setRealm, type DailyLedgerRow, type ProviderStatus, type Realm } from "./lib/commands";
...
<template>
  <main class="app-shell">
    ...
    <section class="metrics" aria-label="今日账本摘要"><article><span>今日净变化</span><strong :class="{ positive: netChange > 0 }">{{ netChange > 0 ? `+${netChange}` : netChange }}</strong><small>通货单位</small></article><article><span>未归因变化</span><strong>{{ unexplainedChange > 0 ? `+${unexplainedChange}` : unexplainedChange }}</strong><small>需要补充收支记录</small></article><article><span>今日记录通货</span><strong>{{ ledgerRows.length }}</strong><small>来自有效快照</small></article></section>
    <section class="workspace"><LedgerTable :rows="ledgerRows.map((row) => ({ currencyId: row.currency_id, netChange: row.net_change, explainedChange: row.explained_change, unattributedChange: row.unattributed_change }))" /><aside><SnapshotForm :currencies="[{ id: 'exalted', name: '崇高石' }]" @submit="recordSnapshot" /><p v-if="savedEntryCount !== null" class="notice">快照已保存：{{ savedEntryCount }} 项通货余额。</p><p v-if="saveError" class="error" role="alert">{{ saveError }}</p></aside></section>
  </main>
</template>
