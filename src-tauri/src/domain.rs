...
pub fn calculate_week(
    ...
    let in_week = |timestamp: &str| {
        date_key(&timestamp[..timestamp.len().min(10)])
            .is_some_and(|date| date >= start && date < start + 7)
    };
    let mut quantities_by_currency: BTreeMap<String, Vec<(&str, u64)>> = BTreeMap::new();
    for snapshot in snapshots.iter().filter(|snapshot| {
        snapshot.status == SnapshotStatus::Valid && in_week(&snapshot.captured_at)
    }) {
        for entry in &snapshot.entries {
            quantities_by_currency
                .entry(entry.currency_id.clone())
    ...
    for adjustment in adjustments
        .iter()
        .filter(|adjustment| in_week(&adjustment.occurred_at))
    {
    ...
    for (currency_id, quantities) in &mut quantities_by_currency {
        quantities.sort_by_key(|(captured_at, _)| *captured_at);
        if quantities.len() >= 2 {
            let net = quantities.last().unwrap().1 as i64 - quantities.first().unwrap().1 as i64;
            let explained = *adjustments_by_currency.get(currency_id).unwrap_or(&0);
            rows.push(CurrencyDayLedger {
                currency_id: currency_id.clone(),
                net_change: net,
                explained_change: explained,
                unattributed_change: net - explained,
            });
        }
    }
    Ok(rows)
